...
class FileManager {
 ...
 public:
  ...
  bool createNewDirCustom(const string& path) {
    try {
      if (create_directory(getCurrentPath() + "/" + path)) {
        return true;
      }
      return false;
    } catch (filesystem_error& err) {
  ...
  vector<string> getFileNamesInCurrentDir() {
    pair<vector<DirVectorData>, vector<string>> dirInfo = grabDirsAndFiles();

    vector<string> fileNames = dirInfo.second;

    return fileNames;
  }

  vector<string> getFolderNamesInCurrentDir() {
    pair<vector<DirVectorData>, vector<string>> dirInfo = grabDirsAndFiles();

    vector<DirVectorData> folders = dirInfo.first;
    vector<string> folderNames;

    for (const DirVectorData& folderData : folders) {
      folderNames.push_back(folderData.path);
    }
