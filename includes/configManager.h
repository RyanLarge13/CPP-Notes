...
private:
  ...
  string getNewUsername(const string &currentUsername) {
    const string newUsername = ioHandler.getInput<string>(
        {{"Type your current username again to cancel and return to the main "
          "menu"}},
...
public:
  ...
  bool changePass(vector<string> &userInfo) {
    ...
    cout << "Okay, let's change your password. To cancel and return to main "
            "menu type in your current password again when asked for a new one"
         << endl;
    const string confirmPass = ioHandler.getInput<string>(
  ...
  bool changeDir(vector<string> &userInfo) {
    ...
    cout << "Okay, let's change the directory name that you store your folders "
            "in"
         << endl;
    ...
    if (confirmation == "Y" || confirmation == "y") {
      ...
      if (confirmNewDirName == "Y" || confirmNewDirName == "y") {
        ...
        exceptionHandler.printPlainError(
            "We failed to update the directory name where we store your notes. "
            "Please double check to see if you have permissions set correctly "
  ...
  void createAccount(ofstream *configFile) {
    ...
    string remoteAccount = ioHandler.getInput<string>(
        {{"This application is a sister application to Native Notes, and "
          "Electron Notes"}},
        "Do you already have an existing account with one of these platforms? "
        "(Y/n): ",
  ...
  void finishCreatingAccount(vector<string> &currentData) {
    ...
    if (length == 5) {
      ...
      if (!newMainDirCreated) {
        system("clear");
        exceptionHandler.printPlainError(
            "It looks like we need permission to your home directory, or this "
