...
void checkForAccount() {
  ...
  if (!config) {
    ...
    // Get the user to crete a new account by building their custom config in
    // the new config file created

    // TODO -----------------
    // 1. Login the user over web, or new to app, login new account completely.
    configManager.createAccount(newConfig);
    newConfig->close();
    delete newConfig;

    // Rerun check for account once everything is established so logic jumps to
    // if (config) ....
    checkForAccount();
    return;
  }
  ...
  if (config) {
    ...
    // 3 indicates the user has no intention of picking up where left off
