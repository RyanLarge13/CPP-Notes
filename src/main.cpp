...
enum UserInfo : int { LOGGED_IN, USERNAME, EMAIL, PASSWORD, PIN, MAIN_DIR };
...
void createNewFile() {
  ...
  if (!newFile) {
    exceptionHandler.printPlainError(
        ...
        "your main directory" +
        userInfo[UserInfo::MAIN_DIR] + "We could not create your new note");
    delete newFile;
    printMenu();
  }
...
void createNewDir() {
  ...
  // directory in
  bool dirCreated = fileManager.createNewDirCustom("/" + newDirName);
...
void openNote() {
  ...
  int selection = ioHandler.getInput<int>(
      {{""}}, "Note #: ", "Please provide a valid input");
...
void openDir() {
  ...
  if (selection == 999) {
    if (nesting > 0) {
      ...
      updateDirInfo();
      if ("/" + backDirName == userInfo[UserInfo::MAIN_DIR]) {
        title = "HOME";
      } else {
        title = backDirName;
      }
...
void deleteFile() {
  if (dirInfo.second.size() < 1) {
    exceptionHandler.printPlainError(
        RED + "You have no files to delete within this directory.\n" +
        ENDCOLOR);
    printNotesOptions();
    return;
  }
...
void checkForNavigationOrFileOpening(string &userInput) {
  ...
  if (found) {
    ...
    fileManager.navDir(fileManager.getCurrentPath() + "/" + name);
    updateDirInfo();
    title = name;
    system("clear");
    printMenu();
  ...
  exceptionHandler.printPlainError(
      ...
      "input " +
      userInput + ". Please attempt another operation or check typing");
  printMenu();
}
...
void printMenu() {
  cout << "Folder: " << RED + fileManager.getCurrentPath() + ENDCOLOR << " "
       << endl
       << endl;
  cout << PURPLE + title + ENDCOLOR << endl << endl;
  ...
  cout << PURPLE + "Folders: " + ENDCOLOR << endl;
  printDirs(false, 0);
  cout << endl << endl;

  cout << PURPLE + "Notes: " + ENDCOLOR << endl;
  printFiles(false, 0);
...
void checkForAccount() {
  ...
  // NOTE: User has never used the app or deleted old account
  if (!config) {
    delete config;

    ofstream *newConfig = configManager.openNewConfig();

    // NOTE: Kills app. Cannot continue with working config
    ...
    // NOTE: Recurse once everything is established so logic
    // jumps passed if (!config)... and continues
    checkForAccount();
    return;
  }

  // NOTE: User most likely already exists or has previously used the
  // application or canceled early if returns false
  bool accountEstablished = configManager.checkForExistingAccount();

  // TODO -----------------------
  // 1. Give the option to start over completely
  ...
  if (!fileManager.navigateDir(userInfo[UserInfo::MAIN_DIR])) {
    // TODO: Make sure you print instructions on how to manually fix this
    // problem before returning out of the program
    exceptionHandler.printPlainError(
        "Please make sure you give the correct access rights to the home "
        "directory or the application will not work");

    // NOTE: Kills app. Cannot work unless mainDir is writable
    return;
  }

  dirInfo = fileManager.grabDirsAndFiles();
  config->close();
  delete config;
